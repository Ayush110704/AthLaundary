...
  // Load and ensure only the latest active login session is stored
  const loadAndRecordLoginActivities = (session) => {
    ...
    const adminEmail = session.email || session.userEmail || 'No email provided';
    const adminName = session.fullName || 'Admin';

    // Filter out any existing entries for this exact user so we don't spam duplicate login rows
    const otherUsersActivities = storedActivities.filter(act => act.email !== adminEmail);

    // Create the single unique current session record
    const currentActivity = {
      id: session.loginTimestamp ? new Date(session.loginTimestamp).getTime() : Date.now(),
      initials: initials,
      name: adminName,
      email: adminEmail,
    ...
    // Put the current user's single session at the very top, followed by other users' history
    const updatedActivities = [currentActivity, ...otherUsersActivities];
